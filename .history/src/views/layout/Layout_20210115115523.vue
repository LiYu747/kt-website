      ...
      <div class="topTil flex al-center ju-center">
        ...
        <div
          ...
          :key="item.id"
           ...
           :style="{color: col}"
        >
           <div class="hov" @click="addTil(item)"  @mouseenter='mousin(index)' :class="{'dvv':item.path == $route.path,
           'dv':$route.path=='/about/pendulumBrake'&&index==2 || $route.path=='/about/wingBrake'&&index==2 || $route.path=='/about/threeGates'&&index==2
           ||$route.path=='/about/panBrake'&&index==2||$route.path=='/about/qualcommBrake'&&index==2||$route.path=='/about/standColumn'&&index==2}"> {{ item.title }}</div>
          <div v-if="index==2 && flag==true" class="showlod flex-d">
                 <div class="  bosshow">
                    <div v-for="items in product"   @click="productCenter(items)" class="itemsshow fz-14" :key="items.id"> 
                      <div class="Til" >
                        {{items.title}}
                      </div>
                  </div>
  ...
  methods: {
    ...
    addTil(item) {
      if (item.heigth) {
        ...
        window.scrollTo(0, item.heigth + a);
        console.log(item.heigth + a))
      }
