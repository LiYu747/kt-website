      ...
      <div class="topTil flex al-center ju-center">
        ...
        >
          <div
            class="hov"
            @click="addTil(item)"
            :class="$route.path == item.path ? 'dv' : ''"
            @mouseenter="mousin(index)"
          >
            {{ item.title }}
          </div>
      ...
      <div class="mapBox">
        <!-- <div class="info">
          ...
          可按住鼠标右键控制地图旋转、修改倾斜角度。
              ...
              <baidu-map :style="{width:'500px',height:'500px'}" class="map" ak="bgB1y9zSNdFt8eWmYbV29vbzgZfMIx7x" 
                     :scroll-wheel-zoom="true" center="{lng:104.076651 , lat: 30.481883}">
                    <!--比例尺控件-->
...
export default {
  ...
  mounted() {
    ...
    // map.centerAndZoom(new BMap.Point(104.076651, 30.481883), 18); // 初始化地图,设置中心点坐标和地图级别
